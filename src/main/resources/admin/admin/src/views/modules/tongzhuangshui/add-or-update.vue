...
export default {
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.shangjiamingcheng!=''&&json.shangjiamingcheng){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/tongzhuangshuipinpai/pinpai`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
	...
	this.ruleForm.xiangqing = this.ruleForm.xiangqing.replace(reg,'../../../springboot68ozj/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `tongzhuangshui/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.tongzhuangshuiCrossAddOrUpdateFlag = false;
                  this.parent.search();
