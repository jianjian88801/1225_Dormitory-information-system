<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="捡拾地点" prop="jianshididian">
          <el-input v-model="ruleForm.jianshididian" 
              placeholder="捡拾地点" clearable  :readonly="ro.jianshididian"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="捡拾地点" prop="jianshididian">
              <el-input v-model="ruleForm.jianshididian" 
                placeholder="捡拾地点" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="捡拾时间" prop="jianshishijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.jianshishijian" 
                type="datetime"
                placeholder="捡拾时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.jianshishijian" label="捡拾时间" prop="jianshishijian">
              <el-input v-model="ruleForm.jianshishijian" 
                placeholder="捡拾时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="认领状态" prop="renlingzhuangtai">
          <el-select v-model="ruleForm.renlingzhuangtai" placeholder="请选择认领状态">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="认领状态" prop="renlingzhuangtai">
	      <el-input v-model="ruleForm.renlingzhuangtai"
  ...
  data() {
	...
	shouji : false,
      ...
      rules: {
          ...
          jianshididian: [
                { required: true, message: '捡拾地点不能为空', trigger: 'blur' },
          ],
          jianshishijian: [
                { required: true, message: '捡拾时间不能为空', trigger: 'blur' },
          ],
          renlingzhuangtai: [
          ],
          xiangximiaoshu: [
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.wupinleibie = true;
          ...
          if(o=='jianshididian'){
            this.ruleForm.jianshididian = obj[o];
	    this.ro.jianshididian = true;
            continue;
          }
          if(o=='jianshishijian'){
            this.ruleForm.jianshishijian = obj[o];
	    this.ro.jianshishijian = true;
            continue;
          }
      ...
      this.$http({
		...
		if(json.shouji!=''&&json.shouji){
          ...
          this.$message.error(data.msg);
            ...
            this.renlingzhuangtaiOptions = "未认领,已认领".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `zhaolingxinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zhaolingxinxi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zhaolingxinxiCrossAddOrUpdateFlag = false;
                  this.parent.search();
